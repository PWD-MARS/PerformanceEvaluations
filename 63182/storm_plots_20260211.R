...
ow2_ref_depth <- sys_invert_elev - key_elevs[4]
...
cs1_monitor_data <- marsFetchLevelData(mars_con,
                                       ...
                                       end_date = eval_end,
                                       sump_correct = FALSE) %>%
  mutate(cs1level_ft = level_ft - cs1_ref_depth) # Add correction factor as needed when CS1 measurements are off
...
ow2_monitor_data <- marsFetchLevelData(mars_con,
                                       ...
                                       end_date = eval_end,
                                       sump_correct = FALSE) %>%
  mutate(ow2level_ft = level_ft - ow2_ref_depth - 0.2) #0.2 ft correction factor to line up OW1 and OW2 data
...
full_data <- right_join(cs1_monitor_data, ow1_monitor_data, by = 'dtime') %>%
  right_join(ow2_monitor_data, by = 'dtime') %>%
  left_join(rain_data, by = 'dtime') %>%
  select(dtime, ow1level_ft, ow2level_ft, cs1level_ft, rainfall_in) %>%
  mutate(rainfall_in = replace_na(rainfall_in, 0))

#rm(cs1_monitor_data, ow1_monitor_data, ow2_monitor_data, rain_data)


# Import event metrics
event_data <- read.csv(paste0(smp_id, '/output/metrics_', eval_end, '.csv')) %>%
  mutate(eventdatastart = as_datetime(eventdatastart), 
         eventdataend = as_datetime(eventdataend)) %>%
  filter(eventdatastart >= eval_start & eventdataend <= eval_end) %>%
...
for(i in 1:length(event_data$gage_event_uid)) {
  ...
  wl_ts <-
    ...
    scale_color_manual(labels = c('CS1', 'OW1', 'OW2'), values = c('darkorange2', 'dodgerblue', 'purple')) + 
    scale_y_continuous(
  ...
  ggsave(paste0(smp_id, '/output/storm_plot_', data_start_date, '.png'))
}
