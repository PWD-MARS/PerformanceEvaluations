...
```{r}
ow2_level <- read_xlsx("data/1140-3-2/OW2/1140-3-2_OW2_25Q1_15min_QAQC_20250710_KR.xlsx",
             sheet = "Final_Import_Site#")

ow2_level_corrected <- ow2_level |>
...
rain <- read_xlsx("data/1140-3-2/OW2/1140-3-2_OW2_25Q1_15min_QAQC_20250710_KR.xlsx",
             sheet = "Rainfall Data")

ow2_level_2 <- read_xlsx("data/1140-3-2/OW2/1140-3-2_OW2_25Q2_15min_QAQC_20250710_KR.xlsx",
             sheet = "Final_Import_Site#")
ow2_level_2_corrected <- ow2_level_2 |>
  mutate(dtime = `Standard Dtime` + lubridate::seconds(1),
         dtime = floor_date(dtime, "minute")) |>
  select(dtime,  `Corrected Water Depth (ft)`) |>
  rename(cwlevel = `Corrected Water Depth (ft)`)
...
rain2 <- read_xlsx("data/1140-3-2/OW2/1140-3-2_OW2_25Q2_15min_QAQC_20250710_KR.xlsx",
             sheet = "Rainfall Data")
...
```{r}
ow2_lvlr <- ow2_level_corrected |>
  mutate(cwlevel = (cwlevel + 1.08)) |>
 left_join(rain, join_by(dtime == dtime)) |>
  na.omit()

ow2_lvlr2 <- ow2_level_2_corrected |>
  mutate(cwlevel = (cwlevel + 1.08)) |>
  left_join(rain2, join_by(dtime == dtime)) |>
  na.omit()

ow2_comb <- rbind(ow2_lvlr, ow2_lvlr2)
```

```{r}
OW2_level <- ggplot(ow2_comb, aes(x = dtime, y = cwlevel)) +
...
rainfall <- ggplot(ow2_comb, aes(y = rainfall)) +
  geom_col(aes(dtime)) + 
  scale_y_continuous(lim = c(0, 1)) +
  labs(
    x = NULL,
    y = "Rainfall (in)",
    title = "1140-3-2 OW2 Water Level Response to Rain Events"
  )
...
```{r}
cowplot::plot_grid(rainfall, OW2_level,
                   nrow = 2,
                   rel_heights = c(1,3),
                   rel_widths = c(1,1) 
                   )
