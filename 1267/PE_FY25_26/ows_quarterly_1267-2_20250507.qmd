...
```{r}
# Adjust elevation value to relative value
...
pre_dates <- tibble::tibble(
  ...
  "cs1_label" = "2025-01-01 00:00:00",
  "cs2_label" = "2025-03-05 06:00:00"
)

post_dates <- tibble::tibble(
  "start" = "2025-08-05 00:00:00",
  "end" = "2025-10-01 00:00:00",
  "cs1_label" = "2025-08-05 06:00:00",
  "cs2_label" = "2025-09-05 06:00:00"
)
```
...
annotate("text", x =  ymd_hms(pre_dates$cs2_label),
  ...
  labs(
    ...
    y = "Water level (ft)",
    color = NULL,
    title = "1267-2-1 Inflow Water Level Response (15-minute interval)"
  )  +
...
# Get legend to combine with CS2 Plot at bottom of cowplot
cs1_legend <- cowplot::get_legend(cs1_plot)

# Hide legend in water level plot to save space
...
annotate("text", x =  ymd_hms(pre_dates$cs2_label),
  ...
  labs(
    ...
    y = "Water level (ft)",
    color = NULL,
    title = "1267-2-1 Outflow Water Level Response (15-minute interval)"
  )  +
  # Color for water levels
   scale_color_manual(values = c(
    "CS1 Water Level" = "cornflowerblue",
...
# Get legend to combine later at bottom of cow plot
...
rainfall_pre <- ggplot(ow_comb |>
  ...
  labs(
    x = NULL,
    y = "Rainfall (in)",
    title = "1267-2-1 Rainfall (3-hour interval)"
  )
  # Combine Rain, CS1, CS2 and legends into one plot
...
# Save CS1 legend separately to combine later
cs1_legend <- cowplot::get_legend(cs1_plot)
...
# Get legend to combine later in cow plot
cs2_legend <- cowplot::get_legend(cs2_plot)

# Hide legend in plot to save space
cs2_plot <- cs2_plot + theme(legend.position = "none")
